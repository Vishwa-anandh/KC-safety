...
export default function SettingsScreen() {
  ...
  useEffect(() => {
    ...
    const timer = window.setTimeout(() => {
      sectionRefs.current.security?.scrollIntoView({ behavior: "smooth", block: "center" });
  ...
  useEffect(() => {
    const observer = new IntersectionObserver((entries) => {
      ...
      if (visible?.target.id.startsWith("settings-")) setActiveSection(visible.target.id.replace("settings-", "") as SettingsSectionId);
    }, { rootMargin: "-18% 0px -64%", threshold: [0.05, 0.2, 0.5] });
  ...
  function sectionVisible(id: SettingsSectionId) {
    ...
    <div className="page-stack settings-page settings-workspace">
      ...
      <div className="settings-shell">
        <aside className="settings-index" aria-label="Settings navigation">
          ...
          <nav>
            {visibleSections.map((section) => {
              ...
              return <button key={section.id} type="button" className={cx(activeSection === section.id && "settings-index__item--active")} onClick={() => showSection(section.id)} aria-current={activeSection === section.id ? "true" : undefined}><span><Icon size={18} /></span><span><strong>{section.label}</strong><small>{section.description}</small></span></button>;
            })}
          </nav>
        ...
        <div className="settings-content">
          {demoEnabled && !normalizedQuery && <InlineMessage tone="info" title="Demo environment">Demo accounts, notification preferences, and passkey metadata are stored in this browser for review. Demo mode can be disabled before the production identity service is connected.</InlineMessage>}

          {visibleSections.length === 0 && <div className="settings-empty"><Search size={26} /><h2>No settings found</h2><p>Try a broader word such as theme, passkey, alert, or setup.</p><Button variant="secondary" onClick={() => setQuery("")}>Clear search</Button></div>}

          {sectionVisible("account") && <section id="settings-account" ref={(node) => { sectionRefs.current.account = node; }} className="settings-card settings-section" aria-labelledby="profile-settings-title">
            <div className="settings-card__heading"><span><UserRound size={21} /></span><div><p className="settings-section__eyebrow">Account</p><h2 id="profile-settings-title">Profile and access</h2><p>Your signed-in account controls your role, work scope, and protected permissions.</p></div><span className="managed-badge"><LockKeyhole size={14} /> Organization managed</span></div>
          ...
          {sectionVisible("notifications") && <section id="settings-notifications" ref={(node) => { sectionRefs.current.notifications = node; }} className="settings-card settings-section" aria-labelledby="notification-settings-title">
            <div className="settings-card__heading"><span><Bell size={21} /></span><div><p className="settings-section__eyebrow">Updates</p><h2 id="notification-settings-title">Notifications</h2><p>Choose which work changes should reach you and how often summaries are prepared.</p></div>{savedAt && <span className="settings-card__saved"><Check size={14} /> Saved</span>}</div>
          ...
          {sectionVisible("security") && <section id="settings-security" ref={(node) => { sectionRefs.current.security = node; }} className={cx("settings-card settings-section", passkeySetupRequested && "settings-card--attention")} aria-labelledby="security-settings-title">
            <div className="settings-card__heading"><span><ShieldCheck size={21} /></span><div><p className="settings-section__eyebrow">Security</p><h2 id="security-settings-title">Passkeys and session</h2><p>Manage passwordless sign-in and review the device using your current session.</p></div><span className={passkeySupported ? "capability capability--ready" : "capability capability--unavailable"}><ShieldCheck size={15} />{passkeySupported ? "Secure connection" : "Unavailable"}</span></div>
          ...
          {sectionVisible("guidance") && <section id="settings-guidance" ref={(node) => { sectionRefs.current.guidance = node; }} className="settings-card settings-section" aria-labelledby="setup-settings-title">
            <div className="settings-card__heading"><span><PlayCircle size={21} /></span><div><p className="settings-section__eyebrow">Learning</p><h2 id="setup-settings-title">Guided setup</h2><p>Review your assigned journey, continue from saved progress, or start again.</p></div><span className={cx("setup-state", completed && "setup-state--complete", skipped && "setup-state--paused")}>{completed ? "Complete" : skipped ? "Paused" : progress > 0 ? "In progress" : "Not started"}</span></div>
          ...
          {sectionVisible("support") && <section id="settings-support" ref={(node) => { sectionRefs.current.support = node; }} className="settings-card settings-section" aria-labelledby="support-settings-title">
            <div className="settings-card__heading"><span><CircleHelp size={21} /></span><div><p className="settings-section__eyebrow">Assistance</p><h2 id="support-settings-title">Help and support</h2><p>Find workflow guidance or contact the EHS&S support team.</p></div></div>
            ...
            <div className="support-note"><ShieldCheck size={19} /><div><strong>Your preferences stay with this browser</strong><span>In this review build, appearance, motion, notifications, guided progress, and demo session details are stored locally. Production policies and access remain organization managed.</span></div></div>
          </section>}
        </div>
      </div>
