...
import { Button, EmptyState, IconButton, InlineMessage, PageHeader, Select } from "../components/UI";
...
function RequirementDialog({ item, sections, onClose, onSave }: { item?: MasterRequirement; sections: string[]; onClose: () => void; onSave: (item: MasterRequirement) => void }) {
  const [draft, setDraft] = useState<MasterRequirement>(item ?? { id: "", title: "", section: sections[0] ?? "", version: "v1", status: "Draft" });
