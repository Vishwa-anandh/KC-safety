        ...
        <div className={cx(importStageClass)}>
          ...
          {step === 3 && <>
            ...
            <div className={cx("grid gap-3 md:grid-cols-2")}>
              ...
              <button type="button" onClick={() => setSiteScope("specific")} className={cx("rounded-xl border p-4 text-left", siteScope === "specific" ? "border-kc-blue-600 bg-kc-blue-50 ring-3 ring-kc-blue-100 dark:bg-kc-blue-950 dark:ring-kc-blue-900" : "border-slate-200 bg-white dark:border-slate-700 dark:bg-slate-900")}><strong className={cx("block text-slate-900 dark:text-slate-100")}>Apply to specific sites</strong><span className={cx("mt-1 block text-sm text-slate-600 dark:text-slate-400")}>Choose the sites this batch of requirements should apply to.</span></button>
            </div>
...
export function AdminRequirementsScreen() {
  ...
  const [activeFramework, setActiveFramework] = useState<SectionKind>("operating-system");
  const inFramework = (item: MasterRequirement) => sectionKindByName.get(item.section) === activeFramework;
  ...
  return (
    <div style={{ paddingInline: "var(--page-gutter)" }} className={cx("page-container w-full pt-5 pb-14 text-slate-900 md:pt-8 md:pb-16 dark:text-slate-100")}>
      ...
      <section id="master-data-panel" role="tabpanel" aria-labelledby={activeFramework === "operating-system" ? "framework-tab-os" : "framework-tab-ps"} className={cx(tableCardClass)}>
