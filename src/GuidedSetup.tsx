...
function WelcomeDialog({ role, canChangeRole, onRoleChange, onStart, onSkip }: { role: UserRole; canChangeRole: boolean; onRoleChange: (role: UserRole) => void; onStart: () => void; onSkip: () => void }) {
  ...
  return <div className="setup-layer"><div className="setup-backdrop" /><section ref={ref} className="setup-welcome" role="dialog" aria-modal="true" aria-labelledby="setup-welcome-title">
    ...
    <div className="setup-welcome__content"><span className="setup-kicker"><PlayCircle size={16} /> Guided setup</span><h1 id="setup-welcome-title">Welcome to your EHS&S workspace</h1><p>The setup highlights each important control and moves to the next page for you. It takes about two minutes.</p><div className="setup-role-current"><span className="avatar">{profile.initials}</span><div><small>Signed in as</small><strong>{profile.name}</strong><span>{profile.label} · {profile.scope}</span></div></div>{canChangeRole && <div className="setup-role-picker"><span>Preview how each user works</span><div>{(Object.values(roleProfiles) as RoleProfile[]).map((item) => { const Icon = item.icon; return <button key={item.id} className={cx(role === item.id && "setup-role-option--selected")} onClick={() => onRoleChange(item.id)} aria-pressed={role === item.id}><Icon size={18} /><span><strong>{item.shortLabel}</strong><small>{tours[item.id].length} steps</small></span>{role === item.id && <Check size={16} />}</button>; })}</div></div>}<div className="setup-welcome__actions"><Button variant="tertiary" onClick={onSkip}>Skip setup</Button><Button variant="primary" icon={<PlayCircle size={18} />} onClick={onStart}>Start guided setup</Button></div></div>
  </section></div>;
}
