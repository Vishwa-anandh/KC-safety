...
| Administration | `GET /sites/:siteId/users`, `POST /sites/:siteId/users`, `PUT /sites/:siteId/users/:userId`, `DELETE /sites/:siteId/users/:userId`, `POST /imports/requirements/validate`, `POST /imports/requirements`, `GET /imports/:batchId`, `POST /imports/:batchId/publish` | Validate and stage a multipart Import Template workbook, retrieve its field-level preview, then explicitly publish its draft batch. |
